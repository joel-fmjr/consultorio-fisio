...
public class AssessmentController {
    ...
    public ResponseEntity<List<AssessmentResponseDTO>> findByPatientId(@PathVariable Long patientId) {
    ...
    public ResponseEntity<AssessmentResponseDTO> create(@Valid @RequestBody AssessmentRequestDTO dto) {
        AssessmentResponseDTO created = service.create(dto);
        return ResponseEntity.status(HttpStatus.CREATED).body(created);
    }

    @PutMapping("/{id}")
    @Operation(summary = "Update assessment", description = "Updates an existing assessment")
    public ResponseEntity<AssessmentResponseDTO> update(@PathVariable Long id, @Valid @RequestBody AssessmentRequestDTO dto) {
